...
public class GameManagerFSM : GameManager
{
    ...
    public void UpdateScore(PlayerData playerData)
    {
        ...
        else if (attackerType == PlayerData.PlayerType.FSM)
        {
            humanScore++;
        }
    }
    ...
    {
        // 게임 시작과 움직임 시작!
        humanPlayerGameObject.GetComponent<Player>().enabled = false;
        enemyPlayerGameObject.GetComponent<Player>().enabled = false;

        yield return new WaitForSecondsRealtime(3.0f);  // 3초 대기
                                                        // "Ready?" 텍스트 표시
        UI.instance.UpdateGameText("Ready?");
        yield return new WaitForSecondsRealtime(2.0f);  // 2초 대기

        // "Fight!" 텍스트 표시
        UI.instance.UpdateGameText("Fight!");
        yield return new WaitForSecondsRealtime(1.0f);  // 1초 대기

        // "Fight!" 텍스트 사라지게 하려면 빈 문자열로 설정
        UI.instance.UpdateGameText("");

        // 게임 시작과 움직임 시작!
        humanPlayerGameObject.GetComponent<Player>().enabled = true;
        enemyPlayerGameObject.GetComponent<Player>().enabled = true;
    }
    ...
    public IEnumerator ShowEnd()
    {
        ...
        {
            // "Player1 Win!" 텍스트 표시
            UI.instance.UpdateGameText("Player1 Win!");
        }
        else // Player2가 이기면
        {
            // "Player2 Win!" 텍스트 표시
            UI.instance.UpdateGameText("Player2 Win!");
        }
        yield return new WaitForSecondsRealtime(2.0f);  // 2초 대기
