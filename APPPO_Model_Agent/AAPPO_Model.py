...
if os_name == 'Windows':
    env_name = f"../Duel_Game/{game}"
    
# 모델 저장 및 불러오기 경로
date_time = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
save_path = f"./saved_models/{game}/APPO/{date_time}"
load_path = f"./saved_models/{game}/APPO/20241114234950"
...
if __name__ == '__main__':
    ...
    for step in range(run_step + test_step):
        ...
        state_B = preprocess(dec_B.obs[POWER_OBS], dec_B.obs[SWORD_OBS], dec_B.obs[STATE_OBS])
        ...
        next_state_B = preprocess(term_B.obs[POWER_OBS], term_B.obs[SWORD_OBS], term_B.obs[STATE_OBS]) if done_B else preprocess(dec_B.obs[POWER_OBS], dec_B.obs[SWORD_OBS], dec_B.obs[STATE_OBS])
